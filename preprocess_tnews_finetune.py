...
def process_one_sent_tnews(sent, label, label_map, tokenizer, split):
    ...
    ids = tokenizer.encode(sent).append(tokenizer.encoder["<eod>"])
...
def preprocess_tnews(data, tokenizer, split):
    # Get the token id of "0", "1", "2", ... "9"
    # num_ids = [tokenizer.encode("选项{}:".format(i))[1] for i in range(10)]

    label_list = get_tnews_labels()
    label_map = get_tews_label_map(label_list)
    ...
    for line in tqdm(lines, desc="Preprocessing {}".format(split)):
        jobj = json.loads(line)
        # for sent in jobj["sentence"]:
        sent = jobj["sentence"]
        label = jobj["label"]
        sample = process_one_sent_tnews(sent, label, label_map, tokenizer, split)
        all_data.extend(sample)

    return all_data


def get_tnews_labels():
    """See base class."""
    labels = []
    for i in range(17):
      if i == 5 or i == 11:
...
def get_tews_label_map(label_list):
    label_map = {}
